...
## ## Code to train model Xception in transfer learning on one vs. similar data sets
## parameter:
...
## <amount immutable layers> chose the number of layers to be set immutable
...
if len(sys.argv) != 7:
    sys.stderr.write(
        'Usage: onevsmost_transfer.py [species], [pad|distort], [train|test], <version>, <weightfile>, <ammount immutable layer>\n')
    sys.exit(1)
else:
    mode = sys.argv[1]
    resize = sys.argv[2]
...
base_model.load_weights('frogsumimodels/Xception_species_distort_TF000/Xception.101.0.952.hdf5')
base_model.layers.pop()
base_model_layers = base_model.output
predictions = Dense(len(labeltonumber), activation='softmax')(base_model_layers)
